...
class PDF2BPMNAgentExecutor(AgentExecutor):
    ...
    def _build_form_generator_base_messages(self) -> List[Dict[str, Any]]:
        ...
        user_guideline = {
            ...
            "content": (
                ...
                "</section>\n"
                "```\n\n"
                "- A section must contain exactly one row-layout element.\n"
                "- A row-layout must always have v-model=\"formValues\" and v-slot=\"slotProps\" (verbatim, not renamed).\n"
                "- Inside a row-layout, you must include exactly one div with class='row'.\n"
                "- Inside that div with class='row', you must include divs with class='col-sm-{number}'.\n"
                ...
                "- Every component's name attribute must be unique, and it must also be bound with v-model=\"slotProps.modelValue['<name>']\" (matching the same key as its name attribute) so it reads/writes the enclosing row-layout's model.\n"
                "- Every name attribute (including on row-layout) must be unique.\n"
    ...
    def _make_fallback_form_html(self) -> str:
        ...
        return (
            "<section>"
            "  <row-layout name='free_input_layout' alias='Free Input' is_multidata_mode='false' v-model=\"formValues\" v-slot=\"slotProps\">"
            "    <div class='row'>"
            "      <div class='col-sm-12'>"
            "        <textarea-field name='free_input' alias='Free Input' rows='5' disabled='false' readonly='false' v-model=\"slotProps.modelValue['free_input']\"></textarea-field>"
            "      </div>"
            "    </div>"
            "  </row-layout>"
            "</section>"
